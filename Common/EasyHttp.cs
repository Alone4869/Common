...
namespace Common
{
	...
	public sealed class EasyHttp
	{
		...
		public static async Task<Dictionary<string,object>> Post(string requestUrl,Dictionary<string,object> headers, Dictionary<string,object>? body, string contentType = "application/json")
		{
			...
			foreach (var header in headers.Where(header => !header.Key.Equals("content-type", StringComparison.CurrentCultureIgnoreCase)))
			{
		...
		/// <returns></returns>
		public static async Task<Dictionary<string,object>> Post(string requestUrl,Dictionary<string,object> headers, string? jsonBody)
		{
			using var client = new HttpClient();
